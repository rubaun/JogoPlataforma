...
public class Player : MonoBehaviour
{
    ...
    [SerializeField] private AudioClip coleta;
    ...
    {
        if(other.CompareTag("Ponto") && !coletado)
        {
            coletado = true;
            other.GetComponent<Animator>().SetTrigger("Sumir");
            Destroy(other.gameObject, 0.5f);
            audioSource.PlayOneShot(coleta);
    ...
    {   
        GameObject.Find("bg").GetComponent<AudioSource>().Stop();
        yield return new WaitForSeconds(0.5f);
        audioSource.PlayOneShot(morte);
        animator.SetBool("EstaVivo", false);
        yield return new WaitForSeconds(morte.length);
        Destroy(gameObject);
    }
